...
from repeat_form_checkers import *
...
def home():
    ...
    db = get_db()
    db.execute('''select *, to_char(task_post_time,'MON-DD-YYYY HH12:MIPM') task_post_time
                    from scout_task_board''')
    tasks = db.fetchall()
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            if repeat_scout_checker(form):
                ...
                try:
                    db.execute('''
                                delete from scout_task_board
                                where tech_name = %s
                               ''', (form.tech_name.data,))

                    flash(f'Task accomplished & Technology {form.tech_name.data} added', 'success')
                except:
                    flash(f'Technology {form.tech_name.data} (out of our task list) added. Please double check your scout if you planned to accomplish a task.', 'success')
...
############ get all tech scout outcomes that needs analysis ##############
    ...
    db.execute('''select
                                log_id,
                                tech_name,
                                to_char(scout_time,'MON-DD-YYYY HH12:MIPM') scout_time,
                                username contributor_name
                            from tech_main_log left join users on contributor = user_id
                            where tech_name in
...
def tech_analytics(tech):
    ...
    db.execute('''
                            select ms_name, milestone_id ms_id
                            from milestones as m join

                                 (select distinct m.ma_std_name
                                  from milestones m
                                  ...
                                  from tech_story_log
                                  where tech_name = %s) ms_left on m.ma_std_name = ms_left.ma_std_name
        ...
        if form.validate_on_submit():
            ...
            if existing_ms:
                ...
                db.execute('''insert into tech_story_log
                              ...
                              values (%s, %s, %s, %s, %s, %s, %s, %s)''', (user['user_id'], tech, datetime.now(), form.story_year.data, form.story_date.data, form.story_content.data, form.milestone.data,  form.sources.data))
...
def task_board_e():
    ...
    if not user:
        return redirect(url_for('login'))
    if user['can_edit'] == 0:
        flash('No access to TechEdits. Please contact administrators', 'danger')
        return redirect(url_for('home'))
    ...
    db.execute('''
                            ...
                            from
                                ...
                                max(to_char(scout_time,'MON-DD-YYYY HH12:MIPM')) latest_scout
                                 from tech_main_log
                                 group by tech_name) tml left join
                                (select distinct tech_name, count(*) story_count, max(to_char(contribute_time,'MON-DD-YYYY HH12:MIPM')) latest_commit
                                 from tech_story_log
...
def view_log(log_s_id, mode = 'analytics'):
    ...
    db = get_db()
    db.execute('''
                            select *, to_char(contribute_time,'MON-DD-YYYY HH12:MIPM') contribute_time_dt
                            from tech_story_log tsl left join milestones m
                            on milestone = ma_std_name
        ...
        if form.validate_on_submit():
            if repeat_story_checker(form):
                ...
                db.execute('''update tech_story_log
                              ...
                              where log_s_id = %s''', (user['user_id'], datetime.now(), form.story_year.data, form.story_date.data, form.story_content.data, form.milestone.data,  form.sources.data, log_s_id))
...
def view_scout(log_id, mode = 'analytics' ):
    ...
    db = get_db()
    db.execute('''
                            select *, to_char(scout_time,'MON-DD-YYYY HH12:MIPM') scout_time_dt
...
def commit_story(log_s_id):
    ...
    if existing_story:
        ...
        return redirect(url_for('view_log', user = user, log_s_id = log['log_s_id'] ))
    ...
    return redirect(url_for('view_all_stories',  user = user, tech=log['tech_name']))
...
@app.route('/final_edit_mode', methods = ['GET', 'POST'])
def final_edit_mode():
    user = get_current_user()
    if not user:
        return redirect(url_for('login'))
    ...
    return render_template('final_edit_mode.html', user = user)


@app.route('/scout_task_board', methods = ['GET', 'POST'])
def scout_task_board():
    """
    A simplified tech_scout.
    """
    user = get_current_user()
    if not user:
        return redirect(url_for('login'))
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            if repeat_scout_checker(form, mode = 'main_table') or repeat_scout_checker(form, mode = 'final_table'):
                flash(f'Scout field: The Technology {form.tech_name.data} already exists. Please check.', 'danger')
            try:
                db.execute('''
                        insert scout_task_board
                            (tech_name, task_post_time, notes)
                        values
                            (%s, %s, %s)
                        ''', (form.tech_name.data, datetime.now(), form.notes.data))
                flash(f'Scout Taskboard Updated', 'success')
            except:
                flash(f'Something is wrong. Please double check.', 'danger')

    return render_template('scout_task_board.html', user = user, tasks = tasks, form = form)

@app.route('/tech_overview', methods = ['GET', 'POST'])
def tech_overview():
    """
    Overview and all edit accesses.
    """
    db = get_db()
    user = get_current_user()
    if not user:
    ...
    db.execute('''
                ...
                ''')
    finalized_scout = db.fetchall()

    return render_template('tech_overview.html', user = user, tasks = finalized_scout)

@app.route("/edit_final_scout/<id>", methods = ['GET', 'POST'])
def edit_final_scout(id):
    ...
    db.execute('''
                            select *
                            from tech_main_final left join tech_wiki_final
                            on id = tech_id
                            where id = %s
                            ''', (id,))
    scout = db.fetchone()
...
@app.route("/view_all_final_stories/<tech>", methods = ['GET', 'POST'])
def view_all_final_stories(tech):
    db = get_db()
    user = get_current_user()
    if not user:
        return redirect(url_for('login'))
    ...
    db.execute('''
                            ...
                            from milestones as m join
                                 ...
                                 (select distinct m.ms_name
                                  ...
                                  select distinct milestone ms
                                  from tech_story_final
                                  where name = %s) ms_left
                                  on m.ms_name = ms_left.ms_name
    ...
    db.execute('''
                    ...
                    from tech_story_final tsf left join milestones m on tsf.milestone = m.ma_std_name
...
def edit_final_log(story_id):
    ...
    db = get_db()
    db.execute('''
                            select *, to_char(story_time, 'YYYY-MM-DD') story_time_s
                            from tech_story_final
                            where story_id = %s
                            ''', (story_id,))
    story = db.fetchone()
    ...
    #
    #         ########### prepare the variables #############################
    #         if not form.story_date.data:
    #             story_time = random_date(form.story_year.data)
    #             exact_time = 0
    #         else:
    #             story_time = (datetime.strptime(str(form.story_year.data) + '/' + str(form.story_date.data), '%Y/%m/%d'))
    #             exact_time = 1
    #
    #
    #         try:
    #             # commit changes
    #             db.execute('''update tech_story_final
    #                           set story_year = %s, story_time = %s, story_content = %s, milestone = %s, sources = %s, exact_time = %s
    #                           where story_id = %s''', (form.story_year.data, story_time, form.story_content.data, form.milestone.data,  form.sources.data, exact_time, story_id))
    #
    #             # we might need some cleaner functions here
    #
    #             flash(f'Tech Story Updated', 'success')
...
@app.route('/update_entry/<table>/<colname>/<idcol>/<row_id>/<type>', methods = ['GET', 'POST'])
@app.route('/update_entry/<table>/<colname>/<idcol>/<row_id>/<type>/<mode>', methods = ['GET', 'POST'])
@app.route('/update_entry/<table>/<colname>/<idcol>/<row_id>/<type>/<mode>/<variable_name>', methods = ['GET', 'POST'])
def update_entry(table, colname, idcol, row_id, type, mode = None, variable_name = None):
    """
    ...
    # current value
    sql = 'select ' + str(colname) + ' from ' + str(table) + ' where ' + str(idcol) + ' = %s'
    db.execute(sql,(row_id,))

    try:
        current_value = db.fetchone()[0]
    except:
        current_value = None
    ...
    if request.method == 'POST':
        if form.is_submitted():
            if type == 'Text' and form.update_text.validate(form):
                update_data = form.update_text.data
            elif type == 'Integer' and form.update_integer.validate(form):
                update_data = form.update_integer.data
...
def final_check_edit():
    ...
    if user['final_editor'] == 0:
        flash('You are not a final editor', 'danger')
        return redirect(url_for('home'))
    # see all recent commit to tech_main

    # check the latest id in final tech_main table and reset

    # insert entries from tech_main to tech_main_final
